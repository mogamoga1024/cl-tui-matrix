    ...
    (let* ((a "K1Я1+aИkawa11ya++a-")
      ...
      (loop
        (multiple-value-bind (h j)
          ...
          (dolist (m c)
            (destructuring-bind (x y k l) m
              (let ((o (color-pair k)))
                (wattron f o)
                (write-string-at-point e (subseq a l (1+ l)) x y)
                (wattroff f o))
              (if (< (1+ y) j)
                  (push (list x (1+ y) k (mod (1+ l) b)) d))
              (if (and (= y 0) (< k b))
                  (push (list x 0 (1+ k) l) d))))
          (setf g (random (1- h)))
          (if (every #'(lambda (n) (or (/= (car n) g) (/= (cadr n) 0))) d)
              (push (list g 0 1 (random b)) d))
          (setf c d d nil)
